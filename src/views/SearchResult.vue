...
export default {
  ...
  methods:{
    handleCurrentChange(val) {
        search(this.keyword,20,val).then(res=>{console.log(res);this.results=res['data']['data'];window.scrollTo(0, 0);}).catch(error=>console.log("search error"));
        console.log(`当前页: ${val}`);
    },
    handleSearchEvent(word,key){
      switch (parseInt(key)) {
        case 4:
          ...
          search(word,20,0).then(res=>{console.log(res['data']);this.results=res['data']['data']||{rows:[]}}).catch(error=>console.log(error)); 
          break;
        case 1:
